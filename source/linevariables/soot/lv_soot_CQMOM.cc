...
void lv_soot_CQMOM::getRhsSrc(const int ipt) {
    ...
    nsvar_v = odtl->odtp->nsvar_v;
    nsvar_s = odtl->odtp->nsvar_s;
    ...
    if(kMe==0) {                                                    // to save cost, compute needed terms for all lv_soot_CQMOM objects using this one.
        ...
        for(int i=iS; i<=iE; i++) {                                    // loop over grid points
            ...
            odtl->odtc->setGasStateAtPt(i);                            // set gas state
            ...
            for (int k=0; k<nsvar_v; k++) {
                mom_matrix[k][0] = odtl->svar[k]->d[i] * odtl->rho->d[i];               // fills first s column (longer than the rest)
                counter++;                                                              // counter increase
            }
            for (int j=1; j<nsvar_s; j++) {
                for (int k=0; k<nsvar_v/2; k++) {
                    mom_matrix[k][j] = odtl->svar[counter]->d[i] * odtl->rho->d[i];     // fills the rest of the s columns
                }
            }

            // set variables needed for adaptive wheeler
            double eabs_v = odtl->io->sootParams["eabs_v"].as<double>();
            double eabs_s = odtl->io->sootParams["eabs_s"].as<double>();
            ...
            double m_min = chem->Cmin*chem->MWc/chem->Na;
            double s_min = pow(6*pow(M_PI,2.0)*chem->MWc*chem->Cmin/chem->rho_soot/chem->Na,2.0/3.0);
            ...
            for (int k=0; k<nused_v*2; k++) {
                for (int j=0; j<nused_s[k]*2; j++) {
                    rrSvar_temp[k][j] = (Mnuc[k][j] + Mgrw[k][j] - Moxi[k][j] + Mcoa[k][j]) / odtl->rho->d[i];
                }
            }
...
double lv_soot_CQMOM::Mkj(double v, double s) {

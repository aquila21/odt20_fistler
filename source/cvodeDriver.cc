 ...
 */
void cvodeDriver::init(odtline *p_odtl, bool p_LincludeRhsMix) {
    ...
    atol = odtl->odtp->cvode_atol; // 1.0E-10;
    rtol = odtl->odtp->cvode_rtol; // 1.0E-4;

    //---------- set the number of equations to solve, and the variable map

    neq = odtl->nTrans;
    for(int i=0, k=0; i<odtl->v.size(); i++)
        if(odtl->v.at(i)->L_transported)
            tVarMap[k++] = odtl->v.at(i);
...
static int RHSF(double t, N_Vector y, N_Vector ydot, void* f_data) {
    ...
    for(int k=0; k<cvd->neq; k++)                         // set the transported vars on odt line
        cvd->tVarMap[k]->d.at(cvd->iC) = NV_Ith_S(y,k);

    cvd->odtl->odtc->setCaseSpecificVars_cvode(cvd->iC);

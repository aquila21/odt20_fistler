...
class HSTpost:
    def __init__(self,drc):
        ...
        self.Srate = self.docs['initParams']['Srate']
    ...
    class gasPhase:
        ...
        def FFT_TKE(self, filename, y):
            ...
            vRlz      = data[:,5]
            wRlz      = data[:,6]
            u         = np.interp(y,yRlz,uRlz)-self.supInst.Srate*y
        ...
        def postProc(self, nRlz):
            ...
            for j in range(nRlz):            
                ...
                for i in range(0,self.supInst.nDt):
                    ...
                    #print(filename)
                    data = np.genfromtxt(filename)                
                    self.kp[i] += np.average(0.5*(np.power(data[:,6]-self.supInst.Srate*data[:,1],2)+np.power(data[:,7],2)+np.power(data[:,8],2)))
